...
class RouterClass
{
    ...
    public function CurrentPage(): string
    {
        ...
        return match (true) {
            str_contains($url, '/form') => "form",
            str_contains($url, '/reglement') => "reglement",
            str_contains($url, '/mentionslegales') => "mentionslegales",
            str_contains($url, '/game') => "game",
            str_contains($url, '/quiz-class') => "quiz-class",
            str_contains($url, '/quiz') => "quiz",
            str_contains($url, '/prompts') => "prompts",
            str_contains($url, '/confidentialite') => "confidentialite",
            str_contains($url, '/') => "home",
            default => "home"
        };
    }
    ...
    /**
     * Load la view appropriée 
     *
     * @param  mixed $target
     * @return void
     */
    public function LoadView(?string $target = null)
    {
        $target ??= $this->currentPage;
        $requestedView = __DIR__ . "/../../views/$target.php";

        if (file_exists($requestedView)) return $requestedView;
